...
from ..core.config import RUNS_DIR, new_run_id, W_ROUGE, W_BLEU, W_BERT, DEFAULT_POLICY_PATH, write_manifest
from ..utils.logging import log_jsonl
from ..mutate.space import clip_cfg
from ..mutate.ops import MUTATION_OPS, all_children_with_ops
...
from ..model.predictor import load_classifier, predict_op
...
def run_learned(dataset_name: str, inputs: List[str], refs: List[str],
                generations: int=3, population_size: int=8, top_k: int=3, patience: int=2,
                policy_path: str = DEFAULT_POLICY_PATH) -> Dict:
    run_id = new_run_id()
    ...
    write_manifest(out_dir, {
        "phase": "learned", "dataset": dataset_name, "generations": generations,
        "population_size": population_size, "top_k": top_k, "patience": patience,
        "weights": weights, "policy_path": policy_path
    })
    clf = load_classifier(policy_path)
    ...
    for gen in range(1, generations+1):
        ...
        for s in survivors:
            ...
            # model-predicted op first
            op_pred = predict_op(clf, parent) if clf else None
            if op_pred and op_pred in MUTATION_OPS:
                kcfg = MUTATION_OPS[op_pred](parent)
                children.append((kcfg, op_pred))

            # plus exploration / other ops
            for (kcfg, op) in all_children_with_ops(parent, [x["cfg"] for x in survivors], n_random=1):
                children.append((kcfg, op))
            ...
            # de-dup & eval
            seen, uniq = set(), []
            for (kcfg, op) in children:
                t = tuple(sorted(kcfg.items()))
                if t not in seen:
                    seen.add(t); uniq.append((kcfg, op))
            for (kcfg, op) in uniq:
                agg = _eval_cfg(dataset_name, inputs, refs, kcfg, gen, f"{s['id']}_g{gen}", evals_path, weights)
                scored.append((kcfg, agg["composite"], agg, op, parent))
                log_jsonl(muts_path, {"event":"mutation","gen":gen,"parent":parent,"child":kcfg,"op":op})
